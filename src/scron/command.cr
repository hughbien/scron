...
class Scron::Command
  ...
  def run
    parser = OptionParser.parse(args) do |parser|
      ...
      parser.on("-e", "--edit", "edit jobs") { edit_jobs; exit }
      parser.on("-h", "--help", "show this help message") { print_help(parser); exit }
      parser.on("-v", "--version", "show version") { print_version; exit }
    end
